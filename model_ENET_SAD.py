...
class ENet_SAD(nn.Module):
    ...
    def __init__(self, input_size, pretrained=True, sad=True, encoder_relu=False, decoder_relu=True, weight_share=True):
        super().__init__()
    ...
    def forward(self, img, seg_gt=None, sad_loss=False):
        # Stage 0 - Initial block
        input_size = img.size()
        ...
        #         print(exist_pred.shape, exist_gt.shape)
...
if __name__ == '__main__':
    ...
    tensor = torch.ones((1, 3, 288, 800)).cuda()
    net = ENet(5).cuda()
    T=0
    for i in range(200):
        a=time.time()
